...
async function setupAlarm() {
  const settings = await chrome.storage.local.get(['checkInterval', 'isEnabled']);
...
async function updateGlobalInterval(services) {
  ...
  for (const s of services) {
    if (s.status === '❌' || s.status === '⚠️') {
      ...
      minInterval = Math.min(minInterval, isInitial ? INTERVALS.FAILURE_INITIAL : INTERVALS.FAILURE_LONG);
    } else if (s.status === '🐢') {
